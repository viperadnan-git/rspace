//! The strip above the listing: back/forward, the locator (a breadcrumb that
//! morphs into search), and the actions (refresh, search toggle, `+` menu).
//!
//! A thin view over its `(Explorer, Workspace)`: navigation and file ops live on
//! the workspace, search data per-tab on the explorer. It owns only the bar's own
//! interactions. One per pane, bound to that pane's explorer for life.
            ...
            .when(!empty, |el| {
                el.child(
                    Button::new(
                        "search-subfolders",
                        "Subfolders",
                        if active { ButtonStyle::Primary } else { ButtonStyle::Secondary },
                    )
...
impl Render for ActionBar {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let (can_back, can_forward) = self
            ...
            .map(|ws| {
                let w = ws.read(cx);
                (w.can_back(cx), w.can_forward(cx))
            })

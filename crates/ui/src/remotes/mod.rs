...
impl RemoteConfigModal {
    ...
    fn answer_question(&mut self, cx: &mut Context<Self>) {
        // Trim to match validate_config/submit_config: continuing a mid-flow
        // question under an untrimmed name would target a remote that was never
        // collision-checked.
        let (mode, name, kind, state) = (
            self.mode,
            self.name.read(cx).text().trim().to_string(),
            self.kind.clone(),
            self.state.clone(),
        );

...
impl Workspace {
    ...
    fn clone_focused_tab(&mut self, window: &mut Window, cx: &mut Context<Self>) -> Tab {
        ...
        if let Some(remote) = remote {
            tab.pane.update(cx, |pane, cx| {
                ...
                pane.history = vec![Location { remote: remote.clone(), path: path.clone(), selected: None }];
                pane.explorer.update(cx, |e, cx| e.show(Some(remote), path, None, cx));
            });
        }
        tab
    }

    /// Swap the two groups left↔right (source becomes destination). Invalidates
    /// the compare, since the direction reversed.
    ...
    /// Open (or close) the sync popover anchored to the status bar. Its anchor
    /// exists only while split, so unsplit it explains itself instead.
    pub(crate) fn toggle_sync_popover(&mut self, cx: &mut Context<Self>) {
        if !self.is_split() {
            self.toast("Split the view to compare two folders", false, cx);
            return;
        }
        self.menu = if self.sync_popover_open() { None } else { Some(ActiveMenu::SyncPopover) };
        cx.notify();
    }
    ...
    pub(crate) fn sync_endpoints(&self, cx: &App) -> Option<(String, String)> {
        if self.groups.len() < 2 {
            return None;
        }
        let label = |p: &Entity<Pane>| {
            let p = p.read(cx);
            match &p.open_remote {
    ...
    fn pane_endpoints(&mut self, action: &str, cx: &mut Context<Self>) -> Option<(String, String, String, String)> {
        ...
        let (lr, lp) = {
            let p = self.groups[0].active_tab().pane.read(cx);
            (p.open_remote.clone(), p.path.clone())
        };
        let (rr, rp) = {
            let p = self.groups[1].active_tab().pane.read(cx);
            (p.open_remote.clone(), p.path.clone())
        };
        match (lr, rr) {
            (Some(lr), Some(rr)) => Some((lr, lp, rr, rp)),
            _ => {
    ...
    pub(crate) fn compare_counts(&self) -> Option<(usize, usize, usize, usize)> {
        self.sync.result.as_ref().map(|entries| {
    ...
    pub(crate) fn toggle_resync(&mut self, cx: &mut Context<Self>) {
        self.sync.bisync_resync = !self.sync.bisync_resync;
        cx.notify();
    }
    ...
    pub(crate) fn start_sync(&mut self, cx: &mut Context<Self>) {
        let (mode, resync) = (self.sync.mode, self.sync.bisync_resync);
    ...
    pub(crate) fn clear_compare(&mut self, cx: &mut Context<Self>) {
        ...
        for group in &self.groups {
            let explorer = group.active_tab().pane.read(cx).explorer.clone();
            explorer.update(cx, |e, cx| e.clear_diff(cx));
        }
        cx.notify();
    }
    ...
    pub(crate) fn resize_split(&mut self, x: f32, window: &Window, cx: &mut Context<Self>) {
        ...
        let dock = if self.dock.panel.is_some() { f32::from(self.dock.width) } else { 0.0 };
    ...
    /// One pane's body: the welcome screen when it has no remote open, else the
    /// pane entity, which renders its own action bar + explorer.
    pub(crate) fn render_pane_column(&self, pane: &Entity<Pane>, cx: &mut Context<Self>) -> AnyElement {
        if pane.read(cx).open_remote.is_none() {

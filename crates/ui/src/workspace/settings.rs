...
impl Workspace {
    pub(crate) fn toggle_palette(&mut self, _: &TogglePalette, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let current_remote = self.open_remote(cx);
    ...
    pub(crate) fn open_settings(&mut self, cx: &mut Context<Self>) {
        ...
        // Re-seed both edit buffers: they commit on Enter, so an abandoned edit
        // would still be on screen.
        let font_px = self.ui_font_size().round() as u64;
        let refresh_secs = self.store.get().refresh_secs;
        self.settings.ui_font_field.update(cx, |f, cx| f.set_value(font_px, cx));
        self.settings.refresh_field.update(cx, |f, cx| f.set_value(refresh_secs, cx));
        self.refresh_storage_size(cx);
        self.fetch_rclone_info(cx);
        cx.notify();
    }

    /// Size the state dirs off the UI thread; it is a recursive walk.
    pub(crate) fn refresh_storage_size(&mut self, cx: &mut Context<Self>) {
        let paths = self.app.paths.clone();
        cx.spawn(async move |this, cx| {
            let size = cx.background_executor().spawn(async move { paths.storage_size() }).await;
            this.update(cx, |this, cx| {
                this.settings.storage_size = Some(size);
                cx.notify();
            })
            .ok();
        })
    ...
    pub(crate) fn ask_confirm(
        ...
        let modal = cx.new(|cx| ConfirmModal::new(title, message, confirm_label, danger, cx));
        self.show_confirm(modal, action, cx);
    }

    /// Like [`Self::ask_confirm`], but accept unlocks only once the user types
    /// `require` verbatim — for irreversible whole-remote actions, where one
    /// stray Return would erase everything.
    pub(crate) fn ask_confirm_typed(
        &mut self,
        title: impl Into<SharedString>,
        message: impl Into<SharedString>,
        confirm_label: impl Into<SharedString>,
        require: impl Into<SharedString>,
        action: impl FnOnce(&mut Self, &mut Context<Self>) + 'static,
        cx: &mut Context<Self>,
    ) {
        let modal = cx.new(|cx| {
            ConfirmModal::new(title, message, confirm_label, true, cx).require_text(require, cx)
        });
        self.show_confirm(modal, action, cx);
    }
    ...
    pub(crate) fn set_refresh(&mut self, secs: u64, cx: &mut Context<Self>) {
        ...
        for group in &self.groups {
            for tab in &group.tabs {
                let explorer = tab.pane.read(cx).explorer.clone();
                explorer.update(cx, |e, _| e.set_refresh(secs));
            }
        }
        cx.notify();
    }

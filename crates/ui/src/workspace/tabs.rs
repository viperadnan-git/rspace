...
impl Workspace {
    ...
    pub(crate) fn tab_title(&self, tab: &Tab, cx: &App) -> String {
    ...
    pub(crate) fn focused_group_changed(&mut self, cx: &mut Context<Self>) {
        // Refresh the cached active explorer/action-bar handles first — every
        // tab/group change funnels here, and `retarget_preview` below reads them.
        self.sync_active_handles(cx);
        self.set_active_polling(cx);
    ...
    fn active_context_changed(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let remote = self.open_remote(cx);
    ...
    pub(crate) fn set_active_polling(&mut self, cx: &mut Context<Self>) {
        for group in &self.groups {
            for (ix, tab) in group.tabs.iter().enumerate() {
                ...
                explorer.update(cx, |e, cx| e.set_active(active, cx));
            }
        }
    }
    ...
    pub(crate) fn drop_into_tab(&mut self, id: usize, dragged: &DraggedEntry, mods: Modifiers, cx: &mut Context<Self>) {
        ...
        let (remote, dir) = {
            let pane = self.groups[g].tabs[ix].pane.read(cx);
            (pane.open_remote.clone(), pane.path.clone())
        };

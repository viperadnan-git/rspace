...
impl TasksPane {
    ...
    fn task_row(&self, d: RowData, actions: AnyElement, cx: &mut Context<Self>) -> AnyElement {
        ...
        let (primary, primary_color, secondary): (SharedString, u32, String) = match &status {
            ...
            TaskStatus::Running if bytes == 0 && removed > 0 => {
                (format!("{removed} removed").into(), FG, format!("{verb} · running…"))
            }
            TaskStatus::Running if bytes == 0 => ("…".into(), FG, format!("{verb} · starting…")),
            ...
            TaskStatus::Done if total > 0 => {
                (human_size(total as i64).into(), FG_SUBTLE, format!("{verb} · {time}"))
            }
            TaskStatus::Done if removed > 0 => {
                (format!("{removed} removed").into(), FG_SUBTLE, format!("{verb} · {time}"))
            }
            TaskStatus::Done => ("Done".into(), FG_SUBTLE, format!("{verb} · {time}")),
        ...
        v_flex()
            ...
            .on_mouse_down(
                ...
                cx.listener(move |this, e: &MouseDownEvent, _, cx| {
                    ...
                    // The selection is a HashSet; sort for a stable order.
                    // Job ids are monotonic, so this is submission order.
                    let mut ids: Vec<usize> = this.sel.iter().copied().collect();
                    ids.sort_unstable();
                    let pos = e.position;

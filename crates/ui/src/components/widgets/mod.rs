...
use gpui::{
    div, prelude::*, px, rems, rgb, rgba, svg, Animation, AnimationExt as _, AnyView, App,
    ClickEvent, Context, Div, ElementId, Entity, FocusHandle, FontWeight, HighlightStyle,
    MouseButton, MouseDownEvent, PathPromptOptions, Pixels, Rems, Render, SharedString,
...
pub fn icon_button(id: impl Into<gpui::ElementId>, icon: &'static str) -> Stateful<Div> {
    tinted_icon_button(id, icon, FG_MUTED)
}

fn tinted_icon_button(id: impl Into<gpui::ElementId>, icon: &'static str, tint: u32) -> Stateful<Div> {
    h_flex()
        .id(id)
        .size(rem(22.0))
        .flex_shrink_0()
        ...
        .child(svg().path(icon).size(rem(14.0)).text_color(rgb(tint)))
}

/// A copy button that acknowledges the copy: a green check while `copied`.
/// Drive it with `Workspace::copy_with_feedback`.
pub fn copy_button(id: impl Into<gpui::ElementId>, copied: bool) -> Stateful<Div> {
    if copied {
        tinted_icon_button(id, "icons/check.svg", SUCCESS)
    } else {
        tinted_icon_button(id, "icons/copy.svg", FG_MUTED)
    }
}

...
impl Workspace {
    ...
    fn download_setting(&self, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        setting_block(
            ...
            h_flex()
                ...
                .child(Button::new("choose-dir", "Choose…", ButtonStyle::Secondary).size(ControlSize::Small).on_click(
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        ...
                        .child(Button::new("clear-rclone-cache", "Clear", ButtonStyle::Secondary).size(ControlSize::Small).on_click(
                            cx.listener(|this, _: &ClickEvent, _, cx| this.request_clear_rclone_cache(cx)),
                        )),
                ),
        )
    }
    ...
    fn rclone_path_row(
        ...
        if let Some(input) = editing {
            return h_flex()
                ...
                .child(Button::new(browse_id, "Browse", ButtonStyle::Secondary).size(ControlSize::Small).on_click(cx.listener(|this, _: &ClickEvent, _, cx| this.browse_rclone_edit(cx))))
                .child(Button::new(cancel_id, "Cancel", ButtonStyle::Ghost).size(ControlSize::Small).on_click(cx.listener(|this, _: &ClickEvent, _, cx| this.cancel_rclone_edit(cx))))
                .child(Button::new(save_id, "Save", ButtonStyle::Primary).size(ControlSize::Small).on_click(cx.listener(|this, _: &ClickEvent, _, cx| this.confirm_rclone_edit(cx))))
                .into_any_element();
        }
        ...
        h_flex()
            ...
            .child(
                h_flex()
                    .gap_1()
                    .child(Button::new(change_id, "Change", ButtonStyle::Secondary).size(ControlSize::Small).on_click(
                    ...
                    .when(overridden, |el| {
                        el.child(
                            Button::new(reset_id, "Reset", ButtonStyle::Ghost)
                                .size(ControlSize::Small)
                                .tooltip("Use automatic resolution")
                                .on_click(cx.listener(move |this, _: &ClickEvent, _, cx| this.reset_rclone(field, cx))),
                        )
                    }),
            )
            .into_any_element()
    ...
    fn request_cleanup(&mut self, cx: &mut Context<Self>) {
        self.ask_confirm(
            "Clean up data?",
            "Clears remote and command history, the job log, and old logs. Your preferences and pinned remotes are kept.",
            "Clean up",
            false,
    ...
    /// `clear_history` VACUUMs and the log sweep walks a directory; both block,
    /// so they run off the UI thread.
    fn cleanup_storage(&mut self, cx: &mut Context<Self>) {
        let (db, logs) = (self.app.db.clone(), self.app.paths.logs_dir().to_path_buf());
        self.frequent_remotes.clear();
        cx.spawn(async move |this, cx| {
            cx.background_executor()
                .spawn(async move {
                    db.clear_history();
                    delete_rotated_logs(&logs);
                })
                .await;
            this.update(cx, |this, cx| {
                this.refresh_storage_size(cx);
                this.toast("Cleaned up", false, cx);
                cx.notify();
            })
            .ok();
        })
        ...
        v_flex()
            ...
            .child(
                h_flex()
                    ...
                    .child(Button::new("clean-up", "Clean up", ButtonStyle::Secondary).size(ControlSize::Small).on_click(
                        cx.listener(|this, _: &ClickEvent, _, cx| this.request_cleanup(cx)),
                    )),
            )
    }

...
pub(crate) struct PreviewPane {
    ...
    /// The zoomable image component, reused across subjects.
...
impl PreviewPane {
    pub(crate) fn new(
        ...
        let image = cx.new(|_| ImageView::new());
        Self { workspace, explorer, _obs: obs, service, current: None, image, cache: Vec::new() }
    }
    ...
    fn on_loaded(
        ...
        if self.current.as_ref().is_some_and(|p| p.is(&remote, &path)) {
            self.show_state(&state, cx);
            self.current.as_mut().unwrap().state = state.clone();
            cx.notify();
        }
        self.cache_put(format!("{remote}:{path}"), state);
    }

    /// Feed an image state into the reused [`ImageView`]; other states need no
    /// child update (they render inline).
    fn show_state(&self, state: &PreviewState, cx: &mut Context<Self>) {
        if let PreviewState::Image(image) = state {
            self.image.update(cx, |v, cx| v.show(image.clone(), cx));
        }
    }
...
impl Render for PreviewPane {
    fn render(&mut self, _window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let content = match self.current.as_ref().map(|p| (&p.entry, &p.state)) {
            ...
            Some((_, PreviewState::Image(_))) => self.image.clone().into_any_element(),

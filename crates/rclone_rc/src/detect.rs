...
pub fn detect() -> Result<Rclone, NotFound> {
    // A candidate counts only if it runs: an unexecutable file on PATH must
    // not mask a working binary further down.
    candidates().find_map(|path| version(&path).map(|version| Rclone { path, version })).ok_or(NotFound)
}
...
/// Every plausible rclone, `PATH` first then the well-known install dirs. All
/// of them, not just the first: `detect` runs each until one answers `version`.
fn candidates() -> impl Iterator<Item = PathBuf> {
    let on_path = std::env::var_os("PATH")
        .into_iter()
        .flat_map(|paths| std::env::split_paths(&paths).collect::<Vec<_>>())
        .map(|dir| dir.join(BIN));
    on_path.chain(known_locations()).filter(|c| c.is_file())
}

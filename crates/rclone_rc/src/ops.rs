...
impl Operation {
    ...
    /// `Sync` is `sync/sync`, which deletes everything at the destination that
    /// isn't in the source; the parallel [`crate::SyncMode::Mirror`] registry
    /// reports the same call destructive.
    pub fn destructive(self) -> bool {
        matches!(
            self,
            Operation::Delete | Operation::Cleanup | Operation::Sync | Operation::Rmdir | Operation::Rmdirs
        )
    }
        ...
        match self {
            Operation::Copy | Operation::Move => {
                ...
                Some(if is_dir {
                    ...
                    (method, json!({ "srcFs": format!("{sr}:{sp}"), "dstFs": format!("{dr}:{dst_path}"), "createEmptySrcDirs": true, "deleteEmptySrcDirs": mv }))
                } else {
            ...
            Operation::Sync => {
                ...
                let (dr, dd, _) = dst.as_path()?;
                Some(("sync/sync", json!({ "srcFs": format!("{sr}:{sp}"), "dstFs": format!("{dr}:{dd}"), "createEmptySrcDirs": true })))
            }
            Operation::Delete => {
            ...
            Operation::Cleanup => {
                let [target] = args else { return None };
                let (r, p, is_dir) = target.as_path()?;
                // `fs` must be a directory (rclone 500s on a file).
                let dir = if is_dir { p.to_string() } else { split_parent(p).0 };
                Some(("operations/cleanup", json!({ "fs": format!("{r}:{dir}") })))
            }
            Operation::Rmdir => {
                let [target] = args else { return None };
            ...
            Operation::Rename => {
                ...
                Some(if is_dir {
                    ("sync/move", json!({ "srcFs": format!("{r}:{p}"), "dstFs": format!("{r}:{dst}"), "createEmptySrcDirs": true, "deleteEmptySrcDirs": true }))
                } else {
                    // Keeps movefile where its siblings use the listing-based
                    // `only_file` workaround: the sync engine preserves names, so
                    // it cannot express a rename. Renaming therefore still needs
                    // NewObject, and fails where that can't resolve (torbox).
            ...
            Operation::CopyUrl => {
                ...
                // `autoFilename` overwrites `remote` with the URL's basename,
                // so the destination folder belongs in `fs`.
                Some((
                    "operations/copyurl",
                    json!({ "url": url, "fs": format!("{r}:{d}"), "autoFilename": true }),
                ))
            }
            Operation::SetTier => {
                let [target, tier] = args else { return None };
                let (r, p, is_dir) = target.as_path()?;
                let tier = tier.as_name()?;
                // `fs` must resolve to a directory (rclone 500s on a file), so a
                // single object goes through settierfile.
                Some(if is_dir {
                    ("operations/settier", json!({ "fs": format!("{r}:{p}"), "tier": tier }))
                } else {
                    let (fs, leaf) = fs_leaf(r, p);
                    ("operations/settierfile", json!({ "fs": fs, "remote": leaf, "tier": tier }))
                })
            }
        }
    }
...
impl InfoOp {
    ...
    pub fn build(self, args: &[ArgValue]) -> Option<(&'static str, Value)> {
        ...
        let (r, p, is_dir) = target.as_path()?;
        Some(match self {
            // `fs` must resolve to a directory (rclone 500s on a file), so size a
            // single file by listing its parent restricted to just that entry.
            InfoOp::Size if !is_dir => {
                let (fs, leaf) = fs_leaf(r, p);
                ("operations/size", json!({ "fs": fs, "_filter": { "IncludeRule": [only_file(&leaf)] } }))
            }

...
/// Whether `pid` is a running process. Used to tell a crashed run's leftovers
/// from a live sibling instance's.
#[cfg(unix)]
pub(crate) fn is_running(pid: u32) -> bool {
    is_alive(&pid.to_string())
}

#[cfg(not(unix))]
pub(crate) fn is_running(pid: u32) -> bool {
    // No cmdline means no process on Windows (tasklist lists only live pids).

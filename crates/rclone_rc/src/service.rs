...
use crate::daemon::{Daemon, SharedDaemon, SHUTDOWN_TIMEOUT};
...
impl Service {
    ...
    /// Bounded like the signal path: a hung unmount (dead NFS server) or a
    /// daemon that won't exit must not stall quit indefinitely.
    ...
    pub async fn download(
        ...
        let (method, params) = if is_dir {
            ("sync/copy", serde_json::json!({ "srcFs": format!("{remote}:{path}"), "dstFs": join(&dest, &name), "createEmptySrcDirs": true }))
        } else {
    ...
    pub async fn upload(
        ...
        // `local` is a native OS path, so split with `Path`; `split_parent`
        // only knows the '/'-separated remote form.
        let local_path = std::path::Path::new(&local);
        let name = local_path
            .file_name()
            .map(|n| n.to_string_lossy().into_owned())
            .unwrap_or_else(|| local.clone());
        let parent = local_path.parent().map(|p| p.to_string_lossy().into_owned()).unwrap_or_default();
        let (method, params) = if is_dir {
            (
    ...
    pub async fn compare(
        ...
        // An empty parse must mean "identical", never "not understood": both
        // read as zero differences, and Mirror deletes on that basis.
        const KEYS: [&str; 5] = ["missingOnDst", "missingOnSrc", "differ", "error", "match"];
        if !KEYS.iter().any(|k| v.get(k).is_some()) {
            return Err(ServiceError::Rc(RcError::Status {
                status: 0,
                message: "could not read the comparison result".into(),
            }));
        }
    ...
    pub async fn run_sync(
        ...
        let (method, params) = match mode {
            SyncMode::Copy => ("sync/copy", serde_json::json!({ "srcFs": src_fs, "dstFs": dst_fs, "createEmptySrcDirs": true })),
            SyncMode::Mirror => ("sync/sync", serde_json::json!({ "srcFs": src_fs, "dstFs": dst_fs, "createEmptySrcDirs": true })),
            SyncMode::Bisync => {
